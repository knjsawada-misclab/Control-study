...
- **OFF / 手動 / P / PI / PID** ボタンで制御則を切り替え（OFF はファン停止 → 球は落下）．
- **手動** モードではフィードバックなしで PWM を直接指定（開ループ）．ステップ応答法によるシステム同定に使えます．
- **SP** スライダーで目標高さを変更．
- **KP / TR / TD** スライダーでゲインを調整（標準形 `Kp / Ki / Kd` への換算値も同時表示）．
- PID 選択時は **PI-D / I-PD / 標準PID** のサブモードで制御構造を切り替え可能．
- **球を叩く（外乱）** ボタンで外乱を与え，制御の復帰を確認．
- **むだ時間 L** スライダー（0〜500 ms）でファン指令の遅れを追加し，不安定化の体験が可能．
...
```math
F_{\text{air}}(u, y_{\text{eq}}) = mg \quad\Longrightarrow\quad K_f \, u_{\text{norm}}^{2} \exp\!\left(-\frac{y_{\text{eq}}}{H_d}\right) = mg
```

これを高さについて解くと，平衡高さが得られます．

```math
y_{\text{eq}} = H_d \ln\!\left(\frac{K_f \, u_{\text{norm}}^{2}}{mg}\right)
```
...
```math
\left.\frac{\partial F_{\text{air}}}{\partial y}\right|_{\text{eq}} = -\frac{F_{\text{air}}}{H_d}\bigg|_{\text{eq}} = -\frac{mg}{H_d} \equiv -k
```

```math
\left.\frac{\partial F_{\text{air}}}{\partial u_{\text{norm}}}\right|_{\text{eq}} = \frac{2mg}{u_{\text{norm,eq}}} \equiv b
```
...
```math
G(s) = \frac{Y(s)}{U(s)} = \frac{b}{ms^{2} + cs + k}
```
...
```math
u(t) = K_P\left(e(t) + \frac{1}{T_R}\int_0^t e(\tau)\,d\tau + T_D\,\frac{de}{dt}(t)\right)
```
...
シミュレータでは，PID モード選択時に以下の3つの制御構造を切り替えることができます．ゲイン $K_p$ ， $K_i$ ， $K_d$ は共通で，比例項（P）と微分項（D）の入力信号が異なります．

| 構造 | 比例項 | 積分項 | 微分項 | 特徴 |
| --- | --- | --- | --- | --- |
| **PI-D**（微分先行） | $K_p \cdot e$ | $K_i \int e\,dt$ | $-K_d \cdot d(\text{PV})/dt$ | SP ステップ時の微分キックを防止（デフォルト） |
| **I-PD** | $-K_p \cdot \text{PV}$ | $K_i \int e\,dt$ | $-K_d \cdot d(\text{PV})/dt$ | P も PV に作用し，オーバーシュートをさらに抑制 |
| **標準 PID** | $K_p \cdot e$ | $K_i \int e\,dt$ | $K_d \cdot de/dt$ | 全項が偏差 $e$ に作用（SP 変更時に微分キック発生） |

PI-D と I-PD は微分項を計測値 PV に対して計算するため，目標値 SP をステップ変化させても微分項が暴れません．I-PD はさらに比例項も PV に対して作用させるため，SP 変更時のオーバーシュートが最も小さくなりますが，応答は最も遅くなります．

### むだ時間

シミュレータではファン指令が実際の風量に反映されるまでの**むだ時間** $L$ （0〜500 ms）をオプションで設定できます．むだ時間は制御器の出力をリングバッファで遅延させることで実現しています．

むだ時間が大きいほど位相余裕が減少し，同じゲインでも振動的になりやすく，最終的に不安定化します．これは実機で必ず存在する遅れの影響を体験するための教材的な機能です．
...
P / PI / PID の切り替えは， $K_i$ （積分）・ $K_d$ （微分）項を有効化／無効化することで実現します．OFF はファン停止（ $u = 0$ ）に相当し，球は落下します．**手動**モードではフィードバックなしにスライダーで指定した PWM をそのままプラントに入力する開ループ動作となり，ステップ応答法によるシステム同定に利用できます．
...
| **手動** | 開ループ（PWM直接指定） | PWM依存 | フィードバックなし，ステップ応答の観察用 |
